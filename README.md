if IY_LOADED and not _G.IY_DEBUG == true then
	error("Kitty Admin is already running!",0)
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'Kitty Admin is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Kitty Admin FE v"..ver
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Kitty Admin Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs[#CMDs + 1] = {NAME = 'lol [plr] [speed]', DESC = 'owo'}
CMDs[#CMDs + 1] = {NAME = 'unlol', DESC = 'uwu'}
...
addcmd('lol',{'rape'},function(args, speaker)
	if not r15(speaker) then
		execCmd('unlol')
		wait()
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players)do
			lolAnim = Instance.new("Animation")
			lolAnim.AnimationId = "rbxassetid://148840371"
			lol = speaker.Character.Humanoid:LoadAnimation(lolAnim)
			lol:Play(.1, 1, 1)
			if args[2] then
				lol:AdjustSpeed(args[2])
			else
				lol:AdjustSpeed(3)
			end
			local lolplr = Players[v].Name
			lolDied = speaker.Character:FindFirstChildOfClass'Humanoid'.Died:Connect(function()
				lolLoop:Disconnect()
				lol:Stop()
				lolAnim:Destroy()
				lolDied:Disconnect()
			end)
			lolLoop = game:GetService('RunService').Stepped:Connect(function()
				pcall(function()
					getRoot(Players.LocalPlayer.Character).CFrame = getRoot(Players[lolplr].Character).CFrame
				end)
			end)
		end
	else
...
addcmd('unlol',{'unrape'},function(args, speaker)
	if lolLoop then
		lolLoop:Disconnect()
		lolDied:Disconnect()
		lol:Stop()
